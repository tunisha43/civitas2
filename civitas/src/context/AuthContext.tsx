...
interface AuthContextType {
  ...
  updateUserProfile: (updates: { fullName?: string; phoneNumber?: string; state?: string; city?: string }) => Promise<{ error: string | null }>;
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    // Supabase fires this when a person lands here via a password-reset
    // email link — it establishes a temporary session just for setting a
    // new password, distinct from a normal login.
  ...
  const updateUserProfile = async (updates: { fullName?: string; phoneNumber?: string; state?: string; city?: string }) => {
    if (!user) return { error: 'Not logged in.' };
    try {
      const { data, error: err } = await supabaseSim.db.updateProfile(user.id, updates);
  ...
  const completePasswordReset = async (newPassword: string) => {
    try {
      ...
      // The recovery session is now a full session — load the profile so
      // the person is properly logged in after setting their new password.
      if (data.user) {
        const profileRes = await supabaseSim.db.getProfile(data.user.id);
        if (profileRes.data) {
      ...
      return { error: err.message || 'Failed to set new password.' };
    }
  };
