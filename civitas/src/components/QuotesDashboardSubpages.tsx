...
export const CustomerQuotesDashboardSubpage: React.FC<CustomerQuotesDashboardSubpageProps> = ({ onNavigate, addToast }) => {
  ...
  const handleConfirmEscrowPayment = async () => {
    ...
    if (!selectedQuoteForDetail || !user?.email) {
      addToast('error', 'Not Logged In', 'Please log in to complete this payment.');
      return;
    }
    ...
    try {
      const result = await chargeWithPaystack({
        email: user.email,
        amountNaira: selectedQuoteForDetail.amount,
        metadata: { userId: customerId, type: 'quote_escrow', quoteId: selectedQuoteForDetail.id },
      });
      addToast('success', 'Payment Confirmed', `₦${result.amount.toLocaleString()} secured in escrow.`);
      setEscrowStep(3);
    } catch (err: any) {
