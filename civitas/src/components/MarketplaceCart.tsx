...
import { chargeWithPaystack } from '../lib/paystack';
import { 
...
export const MarketplaceCart: React.FC<MarketplaceCartProps> = ({ isOpen, onClose, addToast, onNavigate }) => {
  ...
  const handleInitiatePaystack = async () => {
    if (!user?.email) {
      addToast?.('error', 'Not Logged In', 'Please log in to complete checkout.');
      return;
    }
    ...
    try {
      const result = await chargeWithPaystack({
        email: user.email,
        amountNaira: grandTotal,
        metadata: { userId, type: 'materials_order' },
      });

      // Payment genuinely verified server-side — now create the order.
      ...
      setStep('success');
      addToast?.('success', 'Payment Successful', `Payment of ₦${result.amount.toLocaleString()} confirmed. Your escrow order was placed successfully.`);
    } catch (err: any) {
      setPaystackLoading(false);
      setStep('summary');
      addToast?.('error', 'Payment Failed', err.message || 'Your payment could not be completed.');
    }
  };
                ...
                <div className="p-5 border border-emerald-500 bg-emerald-50/10 rounded-3xl space-y-4 shadow-inner text-center">
                  <div className="flex flex-col items-center justify-center gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs font-black uppercase text-emerald-600 tracking-wider font-mono">Paystack Secure Checkout</span>
                    </div>
                  </div>
                  ...
                  <div className="py-12 flex flex-col items-center gap-3">
                    ...
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-widest animate-pulse">
                      {paystackLoading ? 'Complete payment in the Paystack window...' : 'Preparing checkout...'}
                    </p>
                  </div>
                </div>
              )}
