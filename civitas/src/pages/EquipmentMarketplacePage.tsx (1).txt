  ...
  const handlePaystackPayment = async () => {
    ...
    try {
      const result = await chargeWithPaystack({
        email: user.email,
        amountNaira: calcs.totalAmount,
        metadata: { userId, type: 'equipment_booking', equipmentId: selectedEq.id },
      });
      ...
      addToast?.('success', 'Booking Confirmed', `Payment of ₦${result.amount.toLocaleString()} confirmed. Escrow locked for "${selectedEq.name}".`);
      loadData();
    } catch (err: any) {
      setPaystackLoading(false);
            ...
            <div className="bg-white dark:bg-slate-800 rounded-3xl border border-gray-100 dark:border-slate-700 w-full max-w-3xl max-h-[90vh] overflow-y-auto p-6 md:p-8 text-left animate-fade-in relative shadow-2xl">
              ...
              {bookingStep === 'billing' && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-xs">
                  ...
                  {/* Right: Paystack Secure Checkout */}
                  <div className="space-y-4">
                    ...
                    {paystackLoading ? (
                      <div className="py-12 flex flex-col items-center gap-2">
                        ...
                        <p className="text-[10px] font-black uppercase tracking-widest text-gray-400 animate-pulse">Complete payment in the Paystack window...</p>
                      </div>
                    ) : (
                      <>
                        <div className="flex gap-2">
