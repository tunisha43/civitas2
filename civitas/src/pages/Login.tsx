...
export const Login: React.FC<LoginProps> = ({ onNavigate, addToast }) => {
  ...
  const handleDemoLogin = async (role: string) => {
    ...
    // 1. Try to sign in
    const res = await signIn({ email: mockEmail });
    if (res.error) {
      // 2. If not registered in local db, sign up
      ...
      // 3. Auto-approve verification code
      const verifyRes = await verifyOtp(mockEmail, '123456');
      if (verifyRes.error) {
        addToast('error', 'Authentication Error', verifyRes.error);
        setDemoLoading(null);
        return;
      }
      addToast('success', 'Demo Account Provisioned', `Crafted and verified standard profile for ${role}.`);
    } else {
