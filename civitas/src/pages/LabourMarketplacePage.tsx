  ...
  const handleConfirmHire = async (e: React.FormEvent) => {
    ...
    try {
      const result = await chargeWithPaystack({
        ...
        metadata: { userId, type: 'labour_booking', workerId: selectedWorker.id },
      });
      ...
      setHireStep('success');
      addToast?.('success', 'Deployment Confirmed', `Payment of ₦${result.amount.toLocaleString()} confirmed and locked in escrow.`);
      loadData();
    } catch (err: any) {
      setBookingLoading(false);
